...
class SimulatorTest(unittest.TestCase):
    ...
    def test_add_agent(self):
        sim = Simulator(5, 5)
        bob = Agent("Bob", 0, 0, 5, 5, decay=0.05, scan_accuracy=0.9, scan_radius=0, scan_falloff=False)
        sim.add_agent(bob)
    ...
    def test_step(self):
        sim = Simulator(4, 3)
        bob = Agent("Bob", 0, 0, 4, 3, decay=0.05, scan_accuracy=0.9, scan_radius=0, scan_falloff=False)
        alice = Agent("Alice", 1, 1, 4, 3, decay=0.05, scan_accuracy=0.9, scan_radius=0, scan_falloff=False)
        sim.add_agent(bob)
        sim.add_agent(alice)
